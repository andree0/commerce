...
def test_add_bid(client, auction, user):
    ...
    data = {"add_bid": True, "user": user.pk, "auction": auction.pk, "price": 100}
    response = client.post(url, data)
    assert response.status_code == 200
...
def test_close_auction(client, auction, user):
    ...
    data = {"close_listings": True}
    response = client.post(url, data)
    assert response.status_code == 200
    assert response.context["auction"].active is False
    assert Auction.objects.filter(active=True).count() == active_auction_before - 1
